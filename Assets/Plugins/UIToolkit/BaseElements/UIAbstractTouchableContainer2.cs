...
/// </summary>
public abstract class UIAbstractTouchableContainer2 : UIAbstractContainer2, ITouchable, IComparable{
	...
	protected float TOUCH_MAX_DELTA_FOR_ACTIVATION = 1; // this is the SD setting. the constructor will modify this for HD/XD
	protected const float CONTENT_TOUCH_DELAY = 0.05f;
	...
	public override float width { get { return _touchFrame.width; } }
	public override float height { get { return _touchFrame.height; } }
	
	
	
	public UIAbstractTouchableContainer2( UILayoutType layoutType, int spacing ) : this( UI.firstToolkit, layoutType, spacing )
	{}
	
	
	// Default constructor
	public UIAbstractTouchableContainer2( UIToolkit manager, UILayoutType layoutType, int spacing ) : base( layoutType )
	{
	...
	/// </summary>
	/// <returns>The back to bounds.</returns>
	/// <param name='elasticityModifier'>lower numbers will snap back slower than higher</param>
	protected IEnumerator springBackToBounds( float elasticityModifier )
	{
		...
		{
			if( layoutType == UIAbstractContainer2.UILayoutType.Horizontal )
		...
		while( !_isDragging )
		{
			...
			var divisor = layoutType == UIAbstractContainer2.UILayoutType.Horizontal ? width : height;
	...
	protected IEnumerator checkDelayedContentTouch()
	{
		...
		if( _isDragging && Mathf.Abs( _deltaTouch ) < TOUCH_MAX_DELTA_FOR_ACTIVATION )
		{
			...
			if( _activeTouchable != null ) _activeTouchable.onTouchBegan( _lastTouch, fixedTouchPosition ); //<----------------------------------------------
		}
	}
	...
	{
		// clip hidden children if theyre sprites.
		foreach( var child in _children )
            if ((UISprite)child != null) clipChild((UISprite)child);
	}
	...
	protected virtual ITouchable getButtonForScreenPosition( Vector2 touchPosition )
	{
	...
	public virtual void onTouchMoved( myTouch touch, Vector2 touchPos ) {}
	...
	/// </summary>
	public override void addChild( params UIObject[] children )
	{
		...
		{
            if (child as UISprite != null && child as UISprite is ITouchable) {
                //((UISprite)child).manager.removeFromTouchables(child as ITouchable);
                ((UISprite)child).manager.removeFromTouchables((UISprite)child as ITouchable);
            }
		}
	}
