...
public abstract class UIAbstractContainer2 : UIObject, IPositionable
{
    ...
    protected List<UIObject> _children = new List<UIObject>();
    public List<UIObject> Children { get { return _children; } }
	...
	public override bool hidden
	{
		...
		set
		{
            ...
            foreach (var child in _children) {
                ...
                if (child as UIAbstractContainer2 != null) {
                    ((UIAbstractContainer2)child).hidden = value;
                }else if(child as UISprite != null){
                    ((UISprite)child).hidden = value;
                } else if (child as UITextInstance != null) {
                    ((UITextInstance)child).hidden = value;
                }
                /*
                // Run appropriate hide method if child is a sprite or container.
                if (((UISprite)child) != null) {
                    ((UISprite)child).hidden = value;
                } else if (((UIAbstractContainer2)child) != null) {
                    ((UIAbstractContainer2)child).hidden = value;
                }*/
            }
		}
	...
	/// </summary>
	public UIAbstractContainer2() : this( UILayoutType.Vertical )
	{}
	...
	/// </summary>
	public UIAbstractContainer2( UILayoutType layoutType )
	{
	...
	/// </summary>
	public virtual void addChild( params UIObject[] children )
	{
	...
	/// </summary>
	public void removeChild( UIObject child, bool removeFromManager )
	{
#if UNITY_EDITOR
        ...
        // if its a sprite and removeFromManager
        if (removeFromManager && ((UISprite)child) != null) {
            ((UISprite)child).manager.removeElement((UISprite)child);
        }
	}
	...
	protected virtual void layoutChildren(){
		...
		if( _layoutType == UIAbstractContainer2.UILayoutType.Horizontal || _layoutType == UIAbstractContainer2.UILayoutType.Vertical ){
			...
			if( _layoutType == UIAbstractContainer2.UILayoutType.Horizontal ){
                ...
                switch (_alignMode) {// Set anchor information
				...
				foreach( var item in _children ){
					...
					// we add spacing for all but the first and last
					if( i != 0 && i != lastIndex ) _contentHeight += _spacing;
		...
		else if( _layoutType == UIAbstractContainer2.UILayoutType.AbsoluteLayout )
		{
			foreach( var item in _children )
			{
				if( !item.hidden )
